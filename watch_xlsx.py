...
import os, sys, glob, time, json, hashlib, atexit, ctypes
...
# ==================== 单实例锁 ====================
LOCK_FILE = os.path.join(REPO_DIR, ".watchdog.lock")
...
def acquire_lock():
    """获取单实例锁（Windows 命名互斥量，真正原子操作），成功返回 True，已有实例运行返回 False"""
    mutex_name = "ChayanDashboardWatchdog_Mutex"
    ERROR_ALREADY_EXISTS = 183
    try:
        kernel32 = ctypes.WinDLL('kernel32', use_last_error=True)
        # CreateMutexW 是 Windows 原子操作，多进程同时调用时只有一个能创建成功
        handle = kernel32.CreateMutexW(None, False, mutex_name)
        last_err = ctypes.get_last_error()
        if last_err == ERROR_ALREADY_EXISTS:
            kernel32.CloseHandle(handle)
            return False
        # 保持互斥量句柄不关闭，进程退出时自动释放
...
def main():
    # 单实例检查：防止多个实例同时运行重复处理文件
    if not acquire_lock():
        log("已有监控实例在运行，本次启动退出", "WARN")
        os._exit(0)
